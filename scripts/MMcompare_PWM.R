...
MMcompare <- function(pwm,query_name, bg, db_motifs, read_order, shuffle_times, pad_underhangs, p_val_limit, min_overlap, alpha, e, outFile, motifNumber ){
  ...
  info_real = get_Scores(pwm,pwm_log_pwm, bg, bg_log_bg, db_motifs, read_order, alpha, min_overlap, p_bg_dist, p_rev_bg_dist, pwm_rev, pwm_rev_log_pwm_rev, pad_underhangs)  
  ...
  for( x in 1:shuffle_times ){
    ...
    info_shuffle = get_Scores(shuff,shuff_log_shuff,bg, bg_log_bg, db_motifs, read_order, alpha, min_overlap, p_bg_dist, p_rev_bg_dist, shuff_rev, shuff_rev_log_shuff_rev, pad_underhangs)
...
for (f in infiles) {
  if( unlist(strsplit(f, "\\."))[-1] == "ihbcp" ){
    ...
    name_parts = unlist(strsplit(unlist(strsplit(basename(f),"\\."))[-2],"_motif_"))
